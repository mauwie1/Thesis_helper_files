...
def preprocess_df(df, var_dict, url_dict, url_dict2, needed_columns, cat_dict, filled_dict):

    df, var_dict = augment_variables(df, var_dict, url_dict, url_dict2)
...
#### Augmentations
##################################################################################################
def augment_variables(frame, var_dict, url_dict, url_dict2):
    ...
    subframe, var_handler = transform_urls(frame["recentlyvieweditems"], var_dict, url_dict, "url_mr")
    frame = pd.concat([frame, subframe], axis=1)
    subframe, var_handler = transform_urls(frame["url-name"], var_dict, url_dict2, "url_heading")
    frame = pd.concat([frame, subframe], axis=1)
